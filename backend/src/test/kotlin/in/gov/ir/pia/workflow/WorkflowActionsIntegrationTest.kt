...
class WorkflowActionsIntegrationTest {
    ...
    fun `when all 9 sections are authenticated record_state becomes AUTHENTICATED`() {
        ...
        val sectionCodes = listOf(
            "srp", "cala", "section_20a", "jmr",
            "section_20d", "section_20e", "section_20f_g",
            "section_20h_i", "mutation",
        )
        ...
        for (code in sectionCodes) {
            post("/api/v1/activity-records/$recordId/submit",      WorkflowActionRequest(sectionCode = code), dyce,  SectionWorkflowStateResponse::class.java)
            post("/api/v1/activity-records/$recordId/verify",      WorkflowActionRequest(sectionCode = code), nodal, SectionWorkflowStateResponse::class.java)
            post("/api/v1/activity-records/$recordId/authenticate", WorkflowActionRequest(sectionCode = code), ce,   SectionWorkflowStateResponse::class.java)
        }

        val recordState = jdbc.queryForObject(
    ...
    fun `workflow history has 3 entries after full SRP section lifecycle`() {
        ...
        val srpInstanceId = jdbc.queryForObject(
            """

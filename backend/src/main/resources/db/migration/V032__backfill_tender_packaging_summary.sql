-- V032: Seed project_activity_summary for existing TENDER_PACKAGING activities.
--
-- Prior to this fix, TENDER_PACKAGING activities were never counted in
-- project_activity_summary because they have no child activity_records and
-- ActivityRecordCreatedEvent was only fired from createRecord(), not create().
-- Each activity row is treated as one "record" in the summary, using the
-- activity's status column as the state.
...
WHERE pa.activity_type_code = 'TENDER_PACKAGING'
